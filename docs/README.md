# NexoWatt Devices – Dokumentation

Alle technischen Markdown-Dateien des Adapters liegen bewusst in diesem Ordner. Der Projektstamm bleibt dadurch übersichtlich; nur die zentrale `README.md` verbleibt dort, damit GitHub und npm die Adapterübersicht korrekt anzeigen.

## Standard und Freigabe

- [Alias Contract v1](ALIAS_CONTRACT_V1_0.5.144.md)
- [Bestandsanlagen-Kompatibilität 0.5.146](LEGACY_COMPATIBILITY_0.5.146.md)
- [Release-Sicherheit](RELEASE_SAFETY.md)
- [Technische Versionshinweise](CHANGELOG.md)
- [Drittanbieterhinweise](THIRD_PARTY_NOTICES.md)

## Wallboxen und Ladepunkte

- [ABL eMH1 – PWM-Stromsteuerung](ABL_PWM_CURRENT_CONTROL_0.5.143.md)
- [ABL eMH1 – exakte Modbus-ASCII-Lesegruppen](ABL_READ_GROUP_FIX_0.5.142.md)
- [Alfen ACE – Control Fix](ALFEN_CONTROL_FIX_0.5.129.md)
- [Alfen ACE – Schreiblogik-Audit](ALFEN_WRITE_AUDIT_0.5.140.md)
- [Alfen ACE – adaptive Schreibadressierung](ALFEN_ADDRESS_COMPATIBILITY_0.5.147.md)
- [Weidmüller Charge Wallbox](WEIDMUELLER_WALLBOX_0.5.137.md)

## Speicher und Sungrow

- [Sungrow – 1-Sekunden-Polling](SUNGROW_1S_POLL_0.5.138.md)
- [Sungrow – Richtungs- und Timing-Fix](SUNGROW_DIRECTION_TIMING_FIX_0.5.133.md)
- [Sungrow – Fast Feedback Fix](SUNGROW_FAST_FEEDBACK_FIX.md)
- [Sungrow – Heartbeat Fix](SUNGROW_HEARTBEAT_FIX_0.5.130.md)
- [Sungrow – Hybridprotokoll V1.1.11](SUNGROW_HYBRID_PROTOCOL_V1.1.11_0.5.131.md)

## Heizung

- [Technische Alternative CMI](TA_CMI_0.5.139.md)
