...
function makeKey(opts) {
  ...
  const responseStart = ((o.responseStart || ':').toString().trim() || ':').charAt(0);
  return `${path}|${baudRate}|${parity}|${dataBits}|${stopBits}|${responseStart}`;
}
...
class ModbusAsciiBus {
  constructor(adapter, opts) {
    ...
    this.opts = {
      ...
      responseStart: ((opts.responseStart || ':').toString().trim() || ':').charAt(0),
    };
  ...
  _onData(chunk) {
    ...
    this._rxText += Buffer.from(chunk).toString('ascii');
    ...
    while (true) {
      const lfIdx = this._rxText.indexOf('\n');
      if (lfIdx < 0) break;
      const rawLine = this._rxText.slice(0, lfIdx + 1);
      this._rxText = this._rxText.slice(lfIdx + 1);

      const line = rawLine.replace(/[\r\n]+$/g, '');
      if (!line) continue;
      const start = line.charAt(0);
      if (start !== this.opts.responseStart) continue;
  ...
  async _waitForFrame(timeoutMs, predicate) {
    ...
    return await new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Modbus ASCII: timeout waiting for response (${to} ms)`));
      }, to);
  ...
  async _transact(unitId, timeoutMs, fc, payloadBytes, parseResponse) {
    return await this._enqueue(async () => {
      ...
      // Drop stale RX state from previous transactions
      this._rxText = '';
      this._frameQueue = [];
