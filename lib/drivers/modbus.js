  ...
  async _writeAlfenControlMirrorWrites(dp, src, raw, unitId, wordOrder, byteOrder, setWriteContext) {
    ...
    for (const mirror of mirrors) {
      ...
      if (!Number.isFinite(addr)) {
        ...
        throw new Error(`Invalid Alfen mirror write address for ${label}`);
      }
      ...
      try {
        ...
        try { this.adapter.log.debug(`[${this.device?.id || 'modbus'}] Alfen mirror write OK: ${label} UID${this._operationUnitId(uid)} FC${fc}@${addr} len=${len} value=${raw}`); } catch (_) {}
      } catch (e) {
        if (optional) {
          try { this.adapter.log.debug(`[${this.device?.id || 'modbus'}] Alfen optional mirror write skipped: ${label}: ${e && e.message ? e.message : e}`); } catch (_) {}
          continue;
        }
        throw e;
      }
  ...
  async _writeAlfenControlAdaptive(dp, src, addr, raw, words, unitId, wordOrder, byteOrder, setWriteContext) {
    ...
    for (const a of candidates) {
      ...
      try {
        ...
        // Optional station/all-IDs mode: after the audited primary write succeeded,
        // mirror the same command to other Alfen server addresses (Socket 2 / SCN)
        // configured on the write source. Mirrors are optional by default, so an
        // unlicensed or single-socket target cannot block the real socket command.
        await this._writeAlfenControlMirrorWrites(dp, src, raw, unitId, wordOrder, byteOrder, setWriteContext);
