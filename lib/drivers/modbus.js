...
class ModbusDriver {
  constructor(adapter, deviceConfig, template, globalConfig) {
    ...
    // Alfen ACE v1.0 (EN 05/2025) is strict: template addresses are real
    // Modbus protocol addresses (document register - 1). Do not learn or reuse
    // alternate +1/table-address variants; those shift 32-bit values into the
    // wrong register pair.
  ...
  _alfenControlCandidateAddresses(dp, src, addr, unitId) {
    ...
    const n = Number(strictAddr);

    // Alfen ACE v1.0 (EN 05/2025) is handled strictly. The template stores
    // protocol addresses, i.e. documentation register minus 1. Never try or reuse
    // a +1/table-address variant for EMS control writes:
    //   Max Current:       document 1210..1211 -> UID1/2 FC16 @1209 len=2
    //   Charge using phase document 1215       -> UID1/2 FC16 @1214 len=1
    //   SCN L1/L2/L3       document 1417/1419/1421 -> UID200 @1416/@1418/@1420
    // Trying @1210/@1215 shifts the write into the neighbouring register and can
    // make the wallbox briefly react and then fall back.
  ...
  async _writeAlfenControlAdaptive(dp, src, addr, raw, words, unitId, wordOrder, byteOrder, setWriteContext) {
    ...
    if (unsupportedUntil && now < unsupportedUntil && !alwaysTryControlWrites && !(this._alfenHints && this._alfenHints.retryAlternateControlAddress !== false)) {
      throw this._alfenUnsupportedError(kind, dpId, addr, unitId, `Retry after ${Math.ceil((unsupportedUntil - now) / 1000)}s.`);
    }

    const len = Math.max(1, Number(words || src?.length || 1));
    const strictAddr = this._alfenStrictControlAddress(dpId, addr);
    if (Number.isFinite(strictAddr) && strictAddr !== Number(addr)) {
      addr = strictAddr;
    }
    ...
    for (const a of candidates) {
      // In audited strict mode we trust the ACE v1 register map and write directly.
      // The optional FC03 preflight can be disabled per template because a transient
      // read/probe failure must not prevent the actual watchdog write command.
      ...
      try {
        ...
        // Station/All-IDs mode: writes must be attempted independently on all configured
        // Alfen server addresses. If ACE is configured for SCN mode, UID200 can be the
        // actual controlling target while UID1/UID2 socket writes are merely readable or
        // accepted-but-not-accounted. Success on any configured mirror is therefore valid.
        const mirrorResult = await this._writeAlfenControlMirrorWrites(dp, src, raw, unitId, wordOrder, byteOrder, setWriteContext);
        if (primaryOk || (mirrorResult && mirrorResult.successes > 0)) {
          if (!primaryOk && mirrorResult && mirrorResult.successes > 0) {
            try {
              this.adapter.log.info(`[${this.device?.id || 'modbus'}] Alfen primary ${dpId} write UID${this._operationUnitId(unitId)}@${a} was rejected, but ${mirrorResult.successes} mirror target(s) accepted value=${raw}.`);
            } catch (_) {}
          }
          return true;
        }

        if (primaryErr) continue;

        // Strict Alfen ACE mode intentionally has no alternate address variant.
        return true;
      } catch (e) {
        lastWriteErr = e;
        if (isModbusIllegalAddressOrValue(e)) continue;
    ...
    if (!hadReadableCandidate) {
      ...
      throw this._alfenUnsupportedError(kind, dpId, addr, unitId, lastProbeErr && lastProbeErr.message ? lastProbeErr.message : 'Strict Alfen ACE protocol address was not readable.');
    }
    ...
    throw this._alfenUnsupportedError(kind, dpId, addr, unitId, 'Strict Alfen ACE protocol address did not accept the write.');
  }
  ...
  _applyAlfenReadAddressVariant(item) {
    // Alfen ACE templates already contain audited protocol addresses.  Do not apply
    // learned/alternate read variants: readback registers are distinct from write
    // registers and must remain on their documented protocol address.
    return Number(item && item.addr);
  }
...
async readDatapoints(datapoints) {
  ...
  async _maybeWriteSungrowSignedPowerControl(dp, value) {
    ...
    // Optional heartbeat for external EMS/VPP modes.  Sungrow expects a timeout
    // value in seconds (normally 20), not a monotonically increasing counter.
    // Newer residential-hybrid firmware can reject the heartbeat while the unit is
    // still in self-consumption/compulsory mode, so templates may request writing
    // the heartbeat after the EMS mode + command block has been accepted.
    const heartbeatTiming = String(cfg.heartbeatTiming || cfg.heartbeatWriteTiming || '').toLowerCase();
    const heartbeatAfterControl = ['after', 'aftercontrol', 'after_control', 'afterblock', 'after_block', 'post', 'postcontrol', 'post_control'].includes(heartbeatTiming);
    if (!heartbeatAfterControl) {
      await this._maybeWriteSungrowHeartbeat(cfg, heartbeatValue);
    }

    const blockOk = await this._maybeWriteSungrowControlBlock(cfg, modeValue, commandValue, powerValue);
    ...
    if (!blockOk) {
      ...
      // Write power before the command.  Several hybrid controllers latch the command
      // together with the current power register; writing command first can leave the
      // device using the previous power value until the command is toggled again.
    ...
    if (heartbeatAfterControl) {
      await this._maybeWriteSungrowHeartbeat(cfg, heartbeatValue);
      if (cfg.repeatCommandAfterHeartbeat === true) {
        await this._writeDatapointByIdOptional(commandDpId, commandValue, 'command after heartbeat');
      }
    }
    ...
    const widePowerDpId = String(cfg.widePowerDpId || cfg.powerWideDpId || cfg.chargeDischargePowerWideDpId || '').trim();
  ...
  async writeDatapoint(dp, value) {
    ...
    const attempt = async () => {
      ...
      // Alfen ACE: handle EMS control writes through the dedicated writer.
      // In the current templates this is strict, not adaptive: protocol addresses are
      // already document-register minus 1 and +1/table-address fallbacks are disabled.
      // The helper still performs the safe FC03 preflight/readability check and caches
      // only the audited address.
