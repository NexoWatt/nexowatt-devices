...
async readDatapoints(datapoints) {
  ...
  async _maybeWriteSungrowSignedPowerControl(dp, value) {
    ...
    // Fast-EMS profiles can disable the synchronous post-command heartbeat so one
    // EMS power write does not become two Modbus writes; the dedicated heartbeat
    // timer still refreshes register 13080 roughly every 10 s.
    const heartbeatAfterCommand = !(cfg.heartbeatAfterPowerCommand === false || cfg.writeHeartbeatAfterPowerCommand === false || cfg.heartbeatOnCommand === false);
    if (heartbeatAfterCommand) {
