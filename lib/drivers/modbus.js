...
class ModbusDriver {
  constructor(adapter, deviceConfig, template, globalConfig) {
    ...
    const alfenHints = (hints && typeof hints.alfen === 'object' && hints.alfen) ? hints.alfen : {};
    ...
    // Some ACE/firmware combinations expose meter registers with the documented
    // register-1 addressing but expose the socket control block at the table address
    // itself.  We cache the first working control address per datapoint so watchdog
    // refreshes keep using the address that the charger actually accepted.
    this._alfenControlAddressByKey = new Map();
    ...
    // Optional Unit-ID fallback/probe support. Some Modbus-TCP devices answer on a
    // vendor-specific Unit-ID even though the TCP endpoint itself is reachable. KEBA
    // KeContact/P40 is a common field example (often UID 255). This is opt-in via
    // template/device hints; once a fallback succeeds, subsequent operations use it.
    const rawUnitIdFallbacks = (c.unitIdFallbacks ?? c.fallbackUnitIds ?? hints.unitIdFallbacks ?? hints.fallbackUnitIds ?? hints.alternateUnitIds ?? []);
    const normalizeUnitIdList = (raw) => {
      const arr = Array.isArray(raw) ? raw : (raw === undefined || raw === null || raw === '' ? [] : [raw]);
      const out = [];
      for (const v of arr) {
        const n = Number(v);
        if (!Number.isFinite(n)) continue;
        const uid = Math.trunc(n);
        // Modbus TCP Unit-ID is one byte; some vendors use 255.
        if (uid <= 0 || uid > 255) continue;
        if (!out.includes(uid)) out.push(uid);
      }
      return out;
    };
    this.unitIdFallbacks = normalizeUnitIdList(rawUnitIdFallbacks);
    if (Number.isFinite(hintUnitId) && hintUnitId > 0 && hintUnitId <= 255 && !this.unitIdFallbacks.includes(Math.trunc(hintUnitId))) {
      this.unitIdFallbacks.unshift(Math.trunc(hintUnitId));
    }
    this._unitIdFallbackWarnUntil = new Map();
    ...
    const rawMinCmdInterval = (c.minCommandIntervalMs ?? hints.minCommandIntervalMs ?? hints.minIntervalMs ?? this.global.modbusMinCommandIntervalMs ?? 50);
    ...
    const rawReconnectBackoff = (c.reconnectBackoffMs ?? hints.reconnectBackoffMs ?? this.global.modbusReconnectBackoffMs ?? 1000);
    ...
    const rawMaxReadRegs = (c.maxReadRegs ?? hints.maxReadRegs ?? hints.maxReadRegisters ?? this.global.modbusMaxReadRegs ?? 60);
  ...
  _hasExplicitSourceUnitId(src) {
    if (!src) return false;
    return src.unitId !== undefined || src.slaveId !== undefined || src.serverAddress !== undefined ||
      src.deviceId !== undefined || src.modbusUnitId !== undefined;
  }

  _unitIdCandidates(unitIdOverride = null, src = null) {
    const primary = this._operationUnitId(unitIdOverride);
    // Do not override explicit per-register Unit-IDs unless the source explicitly allows it.
    if (this._hasExplicitSourceUnitId(src) && src.allowUnitIdFallback !== true) return [primary];
    const out = [primary];
    for (const raw of (this.unitIdFallbacks || [])) {
      const n = Number(raw);
      if (!Number.isFinite(n)) continue;
      const uid = Math.trunc(n);
      if (uid <= 0 || uid > 255) continue;
  ...
  _rememberUnitIdFallback(selectedUid, previousUid, context = '') {
    const sel = Number(selectedUid);
    const prev = Number(previousUid);
    if (!Number.isFinite(sel) || sel <= 0 || sel > 255) return;
    if (Number.isFinite(prev) && sel === prev) return;

    this.manualUnitId = Math.trunc(sel);
    this._setUnitId(Math.trunc(sel));

    const key = String(Math.trunc(sel));
    const now = Date.now();
    const next = this._unitIdFallbackWarnUntil ? (Number(this._unitIdFallbackWarnUntil.get(key)) || 0) : 0;
    if (now >= next) {
      try { if (this._unitIdFallbackWarnUntil) this._unitIdFallbackWarnUntil.set(key, now + 300000); } catch (_) {}
      const ctx = context ? ` (${context})` : '';
      this.adapter.log.info(`[${this.device?.id || 'modbus'}] Modbus Unit-ID fallback selected: UID${Math.trunc(sel)}${Number.isFinite(prev) ? ` instead of UID${Math.trunc(prev)}` : ''}${ctx}.`);
    }
  }
  ...
  _markAlfenControlUnsupported(kind, reason) {
    ...
    const now = Date.now();
    const backoff = Math.max(60000, Number(this._alfenControlUnsupportedBackoffMs || 3600000));
  ...
  _alfenUnsupportedError(kind, dpId, addr, unitId, reason) {
    const err = new Error(
      ...
      `This is a capability/configuration condition, not an address fallback issue.${reason ? ` ${reason}` : ''}`
    );
  ...
  _alfenControlCandidateAddresses(dp, src, addr, unitId) {
    ...
    // Cached working address first.
    if (this._alfenControlAddressByKey && this._alfenControlAddressByKey.has(key)) {
      add(this._alfenControlAddressByKey.get(key));
    }

    add(addr);

    const cfgOffsets = Array.isArray(this._alfenHints && this._alfenHints.controlAddressOffsets)
      ? this._alfenHints.controlAddressOffsets
      : null;
    const offsets = cfgOffsets && cfgOffsets.length ? cfgOffsets : [0, 1];
    for (const offRaw of offsets) {
      const off = Number(offRaw);
      if (Number.isFinite(off)) add(Number(addr) + off);
    }

    // Be conservative: only the ACE control/status blocks have this alternate variant.
    // The template already stores protocol addresses (doc register - 1), but field logs
    // from real chargers show that some firmware profiles answer the writeable block at
    // the table address itself.  We only use that alternate address after a successful
    // FC03 probe on the exact same register span.
    const kind = this._alfenControlKindForDpId(dp && dp.id);
    if (kind === 'socket' || kind === 'scn') add(Number(addr) + 1);
  ...
  async _writeAlfenControlAdaptive(dp, src, addr, raw, words, unitId, wordOrder, byteOrder, setWriteContext) {
    ...
    const now = Date.now();
    const unsupportedUntil = kind === 'scn' ? Number(this._alfenScnUnsupportedUntil || 0) : Number(this._alfenControlUnsupportedUntil || 0);
    // Do not apply the old session-level unsupported latch to direct writes when we can
    // probe alternate addresses.  It is only retained for keepalive throttling after all
    // candidates have failed.
    if (unsupportedUntil && now < unsupportedUntil && !(this._alfenHints && this._alfenHints.retryAlternateControlAddress !== false)) {
      throw this._alfenUnsupportedError(kind, dpId, addr, unitId, `Retry after ${Math.ceil((unsupportedUntil - now) / 1000)}s.`);
    }

    const len = Math.max(1, Number(words || src?.length || 1));
    const candidates = this._alfenControlCandidateAddresses(dp, src, addr, unitId);
    const key = this._alfenControlCacheKey(dp, src, unitId);
    const buf = valueToBuffer(raw, src.dataType, len * 2);
    const regs = bufferToRegs(buf, wordOrder, byteOrder);

    let lastProbeErr = null;
    let lastWriteErr = null;
    let hadReadableCandidate = false;

    for (const a of candidates) {
      // Safe capability check: the Alfen R/W max-current register must be readable with FC03.
      try {
        await this._mbReadHoldingRegisters(a, len, unitId);
        hadReadableCandidate = true;
      } catch (e) {
        lastProbeErr = e;
        if (isModbusIllegalAddressOrValue(e)) continue;
        throw e;
      }

      try {
        if (typeof setWriteContext === 'function') setWriteContext(src, Number(src.fc || 16), a, raw, len, unitId);
        if (Number(src.fc) === 6) {
          await this._mbWriteRegister(a, regs[0], unitId);
        } else {
          await this._mbWriteRegisters(a, regs.slice(0, len), unitId);
        }
        if (this._alfenControlAddressByKey) this._alfenControlAddressByKey.set(key, a);

        // If an alternate control address was accepted, all socket-control readback
        // registers in the same block must use the same variant.  Otherwise adjacent
        // valid registers can be decoded as a false readback (typical symptom: after
        // writing 16 A / 3 phases, the UI snaps back to 0 A / 1 phase).
        const readOffset = Number(a) - Number(addr);
        if (Number.isFinite(readOffset) && readOffset !== 0) {
          this._cacheAlfenReadVariantForDatapoint(dp, unitId, readOffset);
          this._cacheAlfenReadVariantForControlBlock(kind, unitId, readOffset);
        }

        if (a !== Number(addr)) {
          const logKey = `${key}:variant:${a}`;
          const last = Number(this._alfenUnsupportedLogTsByKey.get(logKey) || 0);
          if (!last || Date.now() - last > 300000) {
            this._alfenUnsupportedLogTsByKey.set(logKey, Date.now());
            this.adapter.log.info(`[${this.device?.id || 'modbus'}] Alfen control address variant selected for ${dpId}: UID${this._operationUnitId(unitId)} FC${Number(src.fc || 16)}@${a} (templateAddress=${Number(src.address)}).`);
          }
        }
        return true;
      } catch (e) {
    ...
    if (!hadReadableCandidate) {
      this._markAlfenControlUnsupported(kind, lastProbeErr && lastProbeErr.message ? lastProbeErr.message : String(lastProbeErr || 'all probes failed'));
      throw this._alfenUnsupportedError(kind, dpId, addr, unitId, lastProbeErr && lastProbeErr.message ? lastProbeErr.message : 'No readable Alfen control candidate address found.');
    }

    // At least one candidate was readable but none accepted the written value. Surface the
    // real Modbus write error, but include the candidate list so the next field log is useful.
    if (lastWriteErr) {
      try { lastWriteErr.message = `${lastWriteErr.message || String(lastWriteErr)} | Alfen tried control candidates UID${this._operationUnitId(unitId)}@[${candidates.join(', ')}], value=${raw}.`; } catch (_) {}
      throw lastWriteErr;
    }

    throw this._alfenUnsupportedError(kind, dpId, addr, unitId, 'No Alfen control candidate address accepted the write.');
  }
  ...
  _applyAlfenReadAddressVariant(item) {
    if (!this._isAlfenProfile || !item || !item.dp || !this._isAlfenAdaptiveReadDpId(item.dp.id)) return Number(item && item.addr);
    const key = this._alfenReadVariantKeyForItem(item);
    const off = Number(this._alfenReadAddressOffsetByKey && this._alfenReadAddressOffsetByKey.get(key));
    const base = Number(item.addr);
    if (Number.isFinite(off) && off !== 0 && Number.isFinite(base)) return base + off;
    return base;
  }

  _alfenReadFallbackOffsetsForGroup(g) {
    if (!this._isAlfenProfile || !g || !Array.isArray(g.items) || !g.items.length) return [];
    if (!g.items.every(item => item && item.dp && this._isAlfenAdaptiveReadDpId(item.dp.id))) return [];
    const cfgOffsets = Array.isArray(this._alfenHints && this._alfenHints.controlAddressOffsets)
      ? this._alfenHints.controlAddressOffsets
      : [0, 1];
    const out = [];
    for (const offRaw of cfgOffsets) {
      const off = Number(offRaw);
      if (Number.isFinite(off) && off !== 0 && !out.includes(off)) out.push(off);
    }
    if (!out.includes(1)) out.push(1);
    return out;
  }

  _cacheAlfenReadVariantForGroup(g, unitId, offset) {
    ...
    for (const item of g.items) {
      ...
      if (key && this._alfenReadAddressOffsetByKey) this._alfenReadAddressOffsetByKey.set(key, off);
    }
  }

  _cacheAlfenReadVariantForDatapoint(dp, unitId, offset) {
  ...
  _cacheAlfenReadVariantForControlBlock(kind, unitId, offset) {
    ...
    for (const dp of this.template.datapoints) {
      ...
      this._cacheAlfenReadVariantForDatapoint(dp, unitId, off);
    }
  }
...
async readDatapoints(datapoints) {
    ...
    for (const { fc, unitId, items } of byBus.values()) {
      ...
      for (const g of groups) {
        ...
        try {
          ...
          if (isModbusIllegalAddressOrValue(e)) {
            // First try explicit Unit-ID fallbacks on the same address. This is safer
            // than shifting addresses and fixes devices whose Modbus-TCP endpoint is
            // reachable but exposes data only on a vendor Unit-ID (e.g. KEBA UID 255).
            const unitCandidates = this._unitIdCandidates(unitId, (g.items && g.items[0] && g.items[0].src) || null);
            for (const altUid of unitCandidates.slice(1)) {
              try {
                if (fc === 3) {
                  regs = (await this._mbReadHoldingRegisters(g.start, len, altUid)).data || [];
                } else {
                  regs = (await this._mbReadInputRegisters(g.start, len, altUid)).data || [];
                }
                fallbackReadOk = true;
                this._rememberUnitIdFallback(altUid, unitId, `FC${fc}@${g.start}-${g.end}`);
                break;
              } catch (ue) {
                if (!isModbusIllegalAddressOrValue(ue)) break;
              }
            }
          }
          if (!fallbackReadOk && isModbusIllegalAddressOrValue(e)) {
  ...
  async writeDatapoint(dp, value) {
    ...
    const attempt = async () => {
      ...
      const fallbackUnitIds = this._unitIdCandidates(unitId, src);

      // Alfen ACE: adapt the writeable control/status block address per real charger.
      // Meter values have consistently used documentRegister-1, but some firmware profiles
      // expose the writeable EMS current block at the table address. Probe before every
      // first use and cache the accepted address.
      ...
      if (fc === 6) {
        ...
        let lastErr = null;
        for (const uid of fallbackUnitIds) {
          for (const a of fallbackAddresses) {
            try {
              setWriteContext(src, fc, a, raw, 1, uid);
              await this._mbWriteRegister(a, regs[0], uid);
              this._rememberUnitIdFallback(uid, unitId, `FC6@${a}`);
              return;
            } catch (e) {
              lastErr = e;
              if (!isModbusIllegalAddressOrValue(e)) throw e;
      ...
      if (fc === 16) {
        ...
        for (const uid of fallbackUnitIds) {
          for (const a of fallbackAddresses) {
            try {
              setWriteContext(src, fc, a, raw, words, uid);
              await this._mbWriteRegisters(a, regs, uid);
              this._rememberUnitIdFallback(uid, unitId, `FC16@${a}`);
              return;
            } catch (e) {
              lastErr = e;
              if (!isModbusIllegalAddressOrValue(e)) throw e;
