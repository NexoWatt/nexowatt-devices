...
async readDatapoints(datapoints) {
  ...
  async _maybeWriteSungrowSignedPowerControl(dp, value) {
    ...
    // Optional heartbeat pre-write for external EMS/VPP modes.  Sungrow expects a
    // timeout value in seconds (normally 20), not a monotonically increasing counter.
    // The min-interval gate avoids burst-writing the heartbeat when keepalive/retry
    // logic refreshes the same battery power command several times in quick succession.
