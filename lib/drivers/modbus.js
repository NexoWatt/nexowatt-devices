  ...
  async writeDatapoint(dp, value) {
    ...
    const attempt = async () => {
      ...
      if (fc === 16 && words > 1 && (src.repeatValue === true || src.repeatToLength === true || src.fillRegisters === true)) {
        ...
        setWriteContext(src, fc, addr, raw, words, unitId);
        await this._mbWriteRegisters(addr, Array(words).fill(one), unitId);
        return;
      }
      ...
      if (fc === 6) {
        ...
        setWriteContext(src, fc, addr, raw, 1, unitId);
        await this._mbWriteRegister(addr, regs[0], unitId);
        return;
      }

      if (fc === 16) {
        setWriteContext(src, fc, addr, raw, words, unitId);
        await this._mbWriteRegisters(addr, regs, unitId);
        return;
      }
