...
class ModbusDriver {
  constructor(adapter, deviceConfig, template, globalConfig) {
    ...
    const tplIdLowerForFallback = String(this.template?.id || '').toLowerCase();
    const mfrLowerForFallback = String(this.template?.manufacturer || '').toLowerCase();
    this._disableAddressFallbackOffsets =
      ...
      mfrLowerForFallback === 'alfen' ||
      tplIdLowerForFallback.includes('alfen');
