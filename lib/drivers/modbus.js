...
async readDatapoints(datapoints) {
  ...
  _nextSungrowHeartbeatValue(cfg) {
    ...
    if (mode === 'counter' || raw === 'counter' || raw === '__counter__') {
      const minRaw = Number(cfg?.heartbeatCounterMin ?? 1);
      const maxRaw = Number(cfg?.heartbeatCounterMax ?? 0xFFFF);
      const min = Number.isFinite(minRaw) && minRaw >= 0 ? Math.trunc(minRaw) : 1;
      const max = Number.isFinite(maxRaw) && maxRaw > min ? Math.trunc(maxRaw) : 0xFFFF;
    ...
    if (mode === 'unix' || mode === 'unixmod' || mode === 'time') {
      return Math.trunc(Date.now() / 1000) & 0xFFFF;
    }
  ...
  async _maybeWriteSungrowSignedPowerControl(dp, value) {
    ...
    const chargeValue = Number(cfg.chargeValue ?? 0xBB);
    const dischargeValue = Number(cfg.dischargeValue ?? 0xAA);
    ...
    // Optional heartbeat pre-write for external EMS/VPP modes.  If configured as a
    // counter it changes on every command/refresh, which is safer for watchdog-style
    // EMS registers than writing a constant value forever.
    if (cfg.heartbeatDpId && heartbeatValue !== undefined && heartbeatValue !== null) {
      await this._writeDatapointByIdOptional(cfg.heartbeatDpId, heartbeatValue, 'heartbeat');
    }
