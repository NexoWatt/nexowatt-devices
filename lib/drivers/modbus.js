...
class ModbusDriver {
  ...
  _alfenControlCandidateAddresses(dp, src, addr, unitId) {
    ...
    const cfgOffsets = Array.isArray(this._alfenHints && this._alfenHints.controlAddressOffsets)
      ...
      : null;
    // By default keep the audited protocol address only.  The Alfen document states
    // that the Modbus address is always document register - 1; for Max Current this
    // means document 1210..1211 => FC16 start address 1209, length 2.
    // Writing at 1210 would shift the 32-bit value into document registers 1211..1212
    // and cross into the neighbouring Safe Current block.
