...
async readDatapoints(datapoints) {
  ...
  async _maybeWriteSungrowSignedPowerControl(dp, value) {
    ...
    // complexity in the device adapter:
    //   1) ensure EMS mode (13050=3) only on first command / at a slow refresh interval;
    //   2) write command+power atomically as FC16 over 13051..13052;
    //   3) refresh heartbeat separately as a fixed timeout value.
    ...
    // Backward-compatible fallback for older Sungrow helper profiles that explicitly
    // requested a contiguous mode+command+power block.
    if (!blockOk && cfg.blockModeCommandPower === true && cfg.blockCommandPowerOnly !== true) {
