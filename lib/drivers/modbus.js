...
class ModbusDriver {
  constructor(adapter, deviceConfig, template, globalConfig) {
    ...
    this._depowerControlWriteDepth = 0;
...
async readDatapoints(datapoints) {
  ...
  async _writeDepowerHelper(dpId, value) {
    ...
    await this.writeDatapoint(target, value);
  }
  ...
  async _maybeWriteDepowerV1003Control(dp, value) {
    ...
    if (!cfg || this._depowerControlWriteDepth > 0) return false;
    ...
    this._depowerControlWriteDepth += 1;
    try {
      ...
      this._depowerControlWriteDepth = Math.max(0, this._depowerControlWriteDepth - 1);
    }
  }
  ...
  async writeDatapoint(dp, value) {
    ...
    const attempt = async () => {
      ...
      // writes. The recursive helper calls are guarded by _depowerControlWriteDepth.
      if (await this._maybeWriteDepowerV1003Control(dp, value)) return;
