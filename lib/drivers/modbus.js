...
async readDatapoints(datapoints) {
  ...
  const groupKey = (fc, g) => `${Number(fc)}:${Number(g.start)}:${Number(g.end)}`;
  ...
  const warnOptionalGroupFailure = (fc, g, e) => {
    const key = groupKey(fc, g);
    const now = Date.now();
    ...
    this.adapter.log.warn(`[${this.device?.id || 'modbus'}] Optional Modbus read group failed and will be skipped temporarily: FC${fc} ${g.start}-${g.end}${ids ? ` (${ids}${(g.items || []).length > 8 ? ', ...' : ''})` : ''}: ${msg}`);
  };

  const shouldSkipGroup = (fc, g) => {
    if (!this.continueOnReadError || !this.skipFailedReadGroupsMs) return false;
    const until = this._readGroupSkipUntil.get(groupKey(fc, g)) || 0;
    return until > Date.now();
  };

  const handleGroupReadError = async (fc, g, e) => {
    ...
    if (this.skipFailedReadGroupsMs > 0) {
      this._readGroupSkipUntil.set(groupKey(fc, g), Date.now() + this.skipFailedReadGroupsMs);
    }
    warnOptionalGroupFailure(fc, g, e);
    return null;
  };
    ...
    for (const [fc, items] of byFc.entries()) {
      if (fc === 1) {
        ...
        for (const g of groups) {
          if (shouldSkipGroup(fc, g)) continue;
          const len = g.end - g.start + 1;
          let res;
          try {
            res = await this._mbReadCoils(g.start, len);
          } catch (e) {
            res = await handleGroupReadError(fc, g, e);
            if (!res) continue;
          }
          const bits = res.data || [];
      ...
      if (fc === 2) {
        ...
        for (const g of groups) {
          ...
          let res;
          try {
            res = await this._mbReadDiscreteInputs(g.start, len);
          } catch (e) {
            res = await handleGroupReadError(fc, g, e);
            if (!res) continue;
          }
      ...
      // Registers
      const groups = buildGroups(items, this.maxReadRegs, { allowGaps: !this.strictContiguousReads });
      for (const g of groups) {
        if (shouldSkipGroup(fc, g)) continue;
        const len = g.end - g.start + 1;
        ...
        try {
          ...
          const res = await handleGroupReadError(fc, g, e);
          if (!res) continue;
          regs = res.data || [];
        }
  ...
  async writeDatapoint(dp, value) {
    ...
    const setWriteContext = (src, fc, addr, raw, words) => {
      writeContext = {
        dpId: String(dp?.id || ''),
        fc: Number(fc),
        address: Number(addr),
    ...
    const annotateWriteError = (err) => {
      ...
      if (writeContext && !err._nexowattWriteContextAnnotated) {
        const ctx = `Write ${writeContext.dpId || 'datapoint'} FC${writeContext.fc}@${writeContext.address}` +
    ...
    const attempt = async () => {
      ...
      if (fc === 5) {
        setWriteContext(src, fc, addr, value, 1);
      ...
      if (fc === 16 && words > 1 && (src.repeatValue === true || src.repeatToLength === true || src.fillRegisters === true)) {
        ...
        setWriteContext(src, fc, addr, raw, words);
        await this._mbWriteRegisters(addr, Array(words).fill(one));
        return;
      }
      ...
      if (fc === 6) {
        ...
        setWriteContext(src, fc, addr, raw, 1);
