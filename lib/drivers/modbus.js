...
class ModbusDriver {
  ...
  _alfenControlCandidateAddresses(dp, src, addr, unitId) {
    ...
    const strictProtocol = !!(this._alfenHints && (
      this._alfenHints.strictProtocolWriteAddress === true ||
      this._alfenHints.adaptiveControlAddress === false ||
      this._alfenHints.retryAlternateControlAddress === false
    ));

    // In strict Alfen mode, never reuse an older cached +1/table-address variant.
    // The template already stores protocol addresses (document register - 1):
    //   Max Current:        doc 1210..1211 -> FC16 @1209 len=2
    //   Charge using phase: doc 1215       -> FC6  @1214 len=1
    // Writing at @1210/@1215 shifts into the next register and can look like it was
    // accepted while the charger later falls back.
    if (strictProtocol) {
      add(strictAddr);
      return out;
    }

    // Cached working address first for explicitly non-strict diagnostic profiles only.
    ...
    const cfgOffsets = Array.isArray(this._alfenHints && this._alfenHints.controlAddressOffsets)
      ...
      : null;
    const offsets = cfgOffsets && cfgOffsets.length ? cfgOffsets : [0];
    for (const offRaw of offsets) {
      const off = Number(offRaw);
      if (Number.isFinite(off)) add(Number(strictAddr) + off);
    }
