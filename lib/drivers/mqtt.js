...
class MqttDriver {
  ...
  async _setSubscriptionDiagnostics(results) {
    ...
    if (ok) {
      ...
      const message = `MQTT connected, but required subscriptions were not granted: ${status}. ` +
        'Check the TESVOLT MQTT user read/subscribe ACL for EMS/APIVersion and EMS/V2/#.';
  ...
  _startNoDataWatch() {
    ...
    const timeoutMs = Number.isFinite(timeoutCandidate) && timeoutCandidate > 0
      ...
      : 10000;
    const baselineCount = this.telemetryMessageCount;
    this._noDataTimer = setTimeout(() => {
      this._noDataTimer = null;
      if (!this.connected || this.telemetryMessageCount !== baselineCount) return;
      const message = `MQTT connection and subscriptions are active, but no EMS message was received within ${timeoutMs} ms. ` +
        'Check whether the TESVOLT V2 publisher is enabled and whether the user has read/subscribe ACL for EMS/APIVersion and EMS/V2/#.';
      this._logConnectionError(message);
  ...
  async _recordIncomingTopic(topic) {
    ...
    const telemetry = !outboundOnly && normalized.startsWith('EMS/V2/');
    if (telemetry) this.telemetryMessageCount += 1;
    this.lastMessageAt = Date.now();
    if (telemetry) this._stopNoDataWatch();
  ...
  async _subscribeAll() {
    ...
    for (const result of primaryResults) {
      ...
      const exactDefinitions = [...this.dpsByTopic.keys()]
        ...
        .map((topic) => ({ topic, qos: result.qos, required: true, fallbackFor: result.topic }));
      ...
      if (exactResults.every((entry) => entry.ok)) {
        result.ok = true;
        result.error = '';
  ...
  async _handleMessage(topic, payload) {
    ...
    if (!incoming.outboundOnly && (parsedCount > 0 || derivedCount > 0 || staleCount > 0 || trackingCount > 0 || validKnownMessage)) {
      this._offlineApplied = false;
      try {
        const alive = this.onAlive && this.onAlive();
        if (alive && typeof alive.catch === 'function') alive.catch(() => {});
