...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    if (batteryCats.has(cat)) {
      ...
      const activePowerDp =
        (isFeneconHomeEss && getAnyById('eSS0_ACTIVE_POWER')) ||
      ...
      const loadPowerDp =
        getAnyById('lOAD_POWER', 'cONSUMPTION_POWER') ||
        this._findFirstDatapoint(dp => /^lOAD_.*pOWER$/i.test(String(dp.id || '')) && dp.rw !== 'wo') ||
        this._findFirstDatapoint(dp => /consumption.*power/i.test(String(dp.id || '')) && dp.rw !== 'wo');

      const batteryGridPowerDp =
        getAnyById('gRID_POWER', 'gRID_ACTIVE_POWER') ||
        this._findFirstDatapoint(dp => /^gRID_.*pOWER$/i.test(String(dp.id || '')) && !/(MAX|MIN)$/i.test(String(dp.id || '')) && dp.rw !== 'wo') ||
        this._findFirstDatapoint(dp => /grid.*power/i.test(String(dp.id || '')) && !/(MAX|MIN)$/i.test(String(dp.id || '')) && dp.rw !== 'wo');
      ...
      const balancePowerUnit =
        (loadPowerDp && loadPowerDp.unit) ||
        (batteryGridPowerDp && batteryGridPowerDp.unit) ||
        (pvPowerDp && pvPowerDp.unit) ||
        powerUnit ||
        'W';
      ...
      const computeBatteryAcSplitPower = (values) => {
        ...
        // This fixes ESS templates like FENECON home.ess where separate AC charge/discharge
        // registers are more reliable than the generic net battery power datapoint.
      ...
      const computeFeneconHomeEssBalancePowerW = (values) => {
        if (!isFeneconHomeEss || !values) return undefined;

        const pv = pvPowerDp ? asNumber(values[pvPowerDp.id]) : undefined;
        const load = loadPowerDp ? asNumber(values[loadPowerDp.id]) : undefined;
        const grid = batteryGridPowerDp ? asNumber(values[batteryGridPowerDp.id]) : undefined;

        // FEMS sign convention:
        //   Grid  : import positive / feed-in negative
        //   ESS   : discharge positive / charge negative
        //   Load  : positive consumption
        // => battery = load - grid - production
        if (pv === undefined || load === undefined) return undefined;
        return load - (grid || 0) - pv;
      };

      const computeBatteryDisplayPowerW = (values) => {
        const balance = computeFeneconHomeEssBalancePowerW(values);
        if (balance !== undefined) return balance;
        return computeBatteryAcPowerW(values);
      };

      const computeBatteryDisplaySplitPower = (values) => {
        const p = computeBatteryDisplayPowerW(values);
        if (p === undefined) return { charge: undefined, discharge: undefined };
        return {
          charge: p < 0 ? Math.abs(p) : 0,
          discharge: p > 0 ? p : 0,
      ...
      if (isFeneconHomeEss) {
        add({
          ...
          name: 'Battery power (AC / ESS port)',
          role: 'value.power',
          type: 'number',
          unit: powerUnit || 'W',
          rw: 'ro',
        ...
        add({
          ...
          name: 'Battery power (balance, signed)',
          role: 'value.power',
          type: 'number',
          unit: balancePowerUnit || 'W',
          rw: 'ro',
          kind: 'computed',
          get: (values) => computeFeneconHomeEssBalancePowerW(values),
        });
      }
      ...
      add({
        ...
        role: 'value.power',
        type: 'number',
        unit: isFeneconHomeEss ? (balancePowerUnit || 'W') : (powerUnit || 'W'),
        rw: 'ro',
        kind: 'computed',
        get: (values) => computeBatteryDisplayPowerW(values),
      });
      ...
      add({
        ...
        rw: 'ro',
        kind: 'computed',
        get: (values) => computeBatteryDisplaySplitPower(values).charge,
      });
      ...
      add({
        ...
        rw: 'ro',
        kind: 'computed',
        get: (values) => computeBatteryDisplaySplitPower(values).discharge,
      });
  ...
  _getSetpointKeepaliveConfig() {
    ...
    const targetsRaw = Array.isArray(cfg.targets)
      ? cfg.targets
      : (cfg.setpointDpId && cfg.remainingTimeDpId)
        ? [cfg]
        : [];
    ...
    for (const t of targetsRaw) {
      ...
      if (!setpointDpId || !remainingTimeDpId) continue;
      targets.push({
        setpointDpId,
        remainingTimeDpId,
    ...
    const tick = async () => {
      ...
      try {
        ...
        for (const t of cfg.targets) {
          ...
          const timeDp = this._getDpById(t.remainingTimeDpId);
          if (!setDp || !timeDp) continue;
          ...
          this.adapter.log.debug(`[${this.cfg.id}] SetpointKeepalive: refreshed ${setDp.id}=${setVal} (remainingS=${remainingS})`);
        }
      } catch (e) {
