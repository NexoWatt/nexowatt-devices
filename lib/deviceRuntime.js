...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    const powerDp = this._getDpById('W') || this._findFirstDatapoint(dp => dp.role === 'value.power' && dp.rw !== 'wo');
    if (powerDp) {
      defs.push({
    ...
    const energyDp = this._getDpById('WH') || this._getDpById('TotWhOut') || this._findFirstDatapoint(dp => dp.role === 'value.energy' && dp.rw !== 'wo');
    if (energyDp) {
      defs.push({
    ...
    const statusDp = this._getDpById('Health') || this._getDpById('St') || this._findFirstDatapoint(dp => dp.role === 'indicator.status' && dp.rw !== 'wo');
    if (statusDp) {
      defs.push({
    ...
    if ((this.template && this.template.category) === 'PV_INVERTER') {
      // grid state (raw)
      const gridStateDp = this._getDpById('PVConn') || this._getDpById('PvGriConn') || this._getDpById('GriSwStt');
      if (gridStateDp) {
        defs.push({
      ...
      const limitPctDp = this._getDpById('WMaxLimPct') || this._getDpById('WLimPct') || this._findFirstDatapoint(dp => (dp.unit === '%' || dp.unit === ' %' || dp.unit === '% ') && (dp.rw === 'rw' || dp.rw === 'wo') && /lim/i.test(String(dp.id)));
      if (limitPctDp) {
        defs.push({
      ...
      // power limit enable
      const limitEnaDp = this._getDpById('WMaxLim_Ena') || this._findFirstDatapoint(dp => (dp.type === 'boolean') && (dp.rw === 'rw') && /lim/i.test(String(dp.id)) && /ena/i.test(String(dp.id)));
      if (limitEnaDp) {
        defs.push({
      ...
      // run/stop command - prefer boolean "Conn" else translate OpMod codes
      const connDp = this._getDpById('Conn');
      const opModDp = this._getDpById('OpMod');
  ...
  async handleStateChange(fullId, state) {
    ...
    if (aliasDef) {
      ...
      try {
        ...
        // best-effort: keep underlying datapoint state in sync with the written raw value
        const dpRelId = this.relStateId(dp);
        await this.adapter.setStateAsync(dpRelId, { val: toDev, ack: true }).catch(() => {});
