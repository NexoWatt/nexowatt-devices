...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    if (batteryCats.has(cat)) {
      ...
      const computeBatteryPowerWFallback = (values) => {
        if (!values) return undefined;
      ...
      const computeBatterySplitPower = (values) => {
        ...
        const p = computeBatteryPowerWFallback(values);
      ...
      const computeBatteryPowerW = (values) => {
        const split = computeBatterySplitPower(values);
      ...
      add({
        ...
        rw: 'ro',
        kind: 'computed',
        get: (values) => computeBatterySplitPower(values).charge,
      });
      ...
      add({
        ...
        rw: 'ro',
        kind: 'computed',
        get: (values) => computeBatterySplitPower(values).discharge,
      });
