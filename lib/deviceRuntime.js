...
class DeviceRuntime {
  ...
  async _seedSetpointKeepaliveFromCommandAliases() {
    ...
    if (this._isAlfenTemplate()) {
      const runCurrentRaw = Number(alfenHints.runCurrentA ?? alfenHints.defaultEnableCurrentA ?? 16);
      const runCurrentA = Number.isFinite(runCurrentRaw) && runCurrentRaw > 0 ? Math.round(runCurrentRaw) : 16;
      const stopCurrentRaw = Number(alfenHints.stopCurrentA ?? 0);
      ...
      if (targets.has('sET_CHARGING_CURRENT')) {
        ...
        if (Number.isFinite(memoryCurrent) && memoryCurrent > 0.1) {
          currentSeed = Math.round(memoryCurrent);
          currentSource = 'info.setpoints.sET_CHARGING_CURRENT';
        } else if (Number.isFinite(aliasCurrent) && aliasCurrent > 0.1) {
