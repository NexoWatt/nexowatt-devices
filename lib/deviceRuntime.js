...
class DeviceRuntime {
  ...
  _getRoundingDecimals(dp) {
    ...
    if (getVartaProfile(this.template)) return null;
  ...
  _buildAliasDefinitions() {
    ...
    if (batteryCats.has(cat) && !getVartaProfile(this.template)) {
  ...
  async start() {
    ...
    if (this.cfg.protocol === 'mqtt' || this.cfg.protocol === 'canbus') {
      ...
      // MQTT is considered online only after a real message was received. CANbus
      // keeps its historical transport-connected behaviour.
      const initialConnected = this.cfg.protocol === 'canbus';
      await this._setStateCached(`${this.baseId}.info.connection`, initialConnected, true);
      await this._setStateCached(`${this.baseId}.info.lastError`, '', true);
      await this._updateAliases({}, { connected: initialConnected, lastError: '' }).catch(() => {});
      return;
    }
    ...
    const doPoll = async (dps) => {
      ...
      try {
        ...
        for (const [dpId, rawVal] of Object.entries(values)) {
          ...
          if (!getVartaProfile(this.template)) val = normalizeValueByUnit(val, dp);
  ...
  async _setError(e) {
    ...
    if (lower.includes('illegal data address') || lower.includes('exception code') || lower.includes('illegal function')) {
      ...
      if (isAlfen) {
        ...
        if (getVartaProfile(this.template)) {
          addHint(
            `VARTA public table 14: verify the selected product and firmware table version. ` +
            `Only the literal documented FC3 addresses and optional FC6 scale-factor writes are supported; ` +
            `do not try address offsets, FC16 or charging setpoints with this public profile.`
          );
    ...
    if (lower.includes('illegal data value') || lower.includes('exception 3')) {
      if (getVartaProfile(this.template)) {
        addHint(`VARTA only documents SINT16 scale-factor configuration with FC6, not power commands. Check manufacturer acceptance of the requested exponent; no alternate write function or automatic retry is used.`);
      } else addHint(
    ...
    try {
      if (isTransport && this.driver && typeof this.driver.disconnect === 'function') {
        ...
        if (getVartaProfile(this.template)) {
          // VARTA terminal disconnect cancels the endpoint scheduler. A transient
