...
class DeviceRuntime {
    ...
    if (chargerCats.has(cat)) {
      ...
      try {
        ...
        if (isAlfenAce) {
          ...
          if (mode3Dp) {
            ...
            add({
              ...
              get: (values) => {
                ...
                const p = toNum(values && values[chargingPowerDp && chargingPowerDp.id]);
          ...
          if (setCurrentDp && (setCurrentDp.rw === 'rw' || setCurrentDp.rw === 'wo')) {
            const runAlias = {
            ...
            add({
              ...
              name: 'Charging released by current setpoint',
              role: 'indicator',
              type: 'boolean',
              rw: 'ro',
              kind: 'dp',
  ...
  async _setError(e) {
    ...
    if (lower.includes('illegal data address') || lower.includes('exception code') || lower.includes('illegal function')) {
      ...
      if (isAlfen && (lower.includes('set_charging_current') || lower.includes('fc16@1209') || lower.includes('1199-1214'))) {
        addHint(
          `Alfen-specific: the socket control/status block around registers 1200-1215 is not exposed by this charger in the current ACE configuration. ` +
