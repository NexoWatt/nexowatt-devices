...
class DeviceRuntime {
  ...
  async _setHeartbeatOnline(nextOnline) {
    ...
    if (!b && String(this.cfg?.protocol || '').toLowerCase() === 'mqtt' && this.driver && typeof this.driver.handleOffline === 'function') {
      await this.driver.handleOffline('MQTT heartbeat timeout').catch(() => {});
    }
  ...
  async _handleMqttSnapshot(values, meta) {
    ...
    const lastError = connected ? '' : String(context.error || 'MQTT data stale or disconnected');
  ...
  async _handleMqttConnectionState(connected, errorMessage) {
    ...
    await this._setHeartbeatOnline(false).catch(() => {});
