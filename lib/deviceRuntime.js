...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    if (batteryCats.has(cat)) {
      ...
      // --- Core read signals ---
      const socDp =
        // Prefer "instant" SoC inputs if available (some devices expose a stale/holding SoC with 0xFFFF)
        getAnyById('eSS_SOC_IN', 'bATTERY_SOC_IN', 'sOC_IN', 'bATTERY_SOC', 'bATTERY_TOTAL_SOC', 'sOC') ||
        findByIdRe(/(^|_)soc_in($|_)/i) ||
