...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    if (chargerCats.has(cat)) {
      ...
      if (controlCommandDp && statusCodeDp) {
        add({
          relId: this._aliasRelId('ctrl.run'),
    ...
    const doPoll = async () => {
      ...
      try {
        ...
        for (const [dpId, rawVal] of Object.entries(values)) {
          ...
          // Optional rounding of numeric values (e.g. meters with 2 decimals).
          const decimals = dp.decimals;
  ...
  async _setError(e) {
    ...
    try {
      ...
      const isTransport = transportCodes.has(code) || lower.includes('port not open');
