...
class DeviceRuntime {
  ...
  async _initHeartbeatAliasObjects() {
    ...
    for (const def of defs) {
      for (const relId of this._hbRelIds(def.name)) {
        const standard = relId.includes(`.aliases.${ALIAS_STANDARD_NAMESPACE}.`);
        ...
        if (def.unit && standard) common.unit = def.unit;
        ...
        const native = {
          ...
          isAlias: true,
          aliasKind: 'heartbeat',
          aliasContractVersion: standard ? ALIAS_SCHEMA_VERSION : undefined,
          aliasContractPath: standard ? `r.${def.name}` : undefined,
          capability: standard ? `read.${def.name}` : undefined,
          deviceClass: standard ? this.aliasDeviceClass : undefined,
        };
  ...
  _buildAliasDefinitions() {
    ...
    const evChargerCats = new Set(['EVCS', 'EVSE']);
    const solarChargerCats = new Set(['CHARGER', 'DC_CHARGER']);
    ...
    const allowGenericRoleAliases = !evChargerCats.has(cat) && !solarChargerCats.has(cat) && !['METER', 'BATTERY', 'ESS', 'BATTERY_INVERTER'].includes(cat);
    ...
    // --- Solar / DC charger aliases (CHARGER / DC_CHARGER) ---
    // These categories contain PV/battery chargers (GoodWe/Victron), not EV charging
    // points. Keeping them separate prevents the UI from listing a solar charger as a
    // wallbox and avoids exposing EV-specific current-limit controls on the wrong device.
    if (solarChargerCats.has(cat)) {
      const powerDp =
    ...
    // --- Charging station aliases (EVCS/EVSE only) ---
    if (evChargerCats.has(cat)) {
      // Read: power
    ...
    // Add canonical legacy synonyms without removing any existing datapoint. This keeps
    // current installations compatible while ensuring the most important names exist at
    // the traditional aliases.* path as well.
    const compatibilityDefs = createLegacyCanonicalSynonyms(defs, {
      baseId: this.baseId,
      deviceClass: this.aliasDeviceClass,
    });
    for (const def of compatibilityDefs) add(def);
  ...
  async registerDynamicAlias(def, options = {}) {
    ...
    await this.adapter.setObjectNotExistsAsync(relId, {
      ...
      native: {
        ...
        aliasContractVersion: def.aliasContractVersion,
        aliasContractPath: def.aliasContractPath,
        capability: def.capability,
        deviceClass: def.aliasContractVersion ? this.aliasDeviceClass : undefined,
      }
    });
    await this.adapter.extendObjectAsync(relId, {
      common,
