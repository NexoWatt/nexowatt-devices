...
class DeviceRuntime {
  constructor(adapter, deviceConfig, template, globalConfig) {
    ...
    // ABL eMH1 controls the normal charging release through IEC 61851 PWM.
    // 100 % means "no current allowed". Keep the last active PWM value separately
    // so a pause does not destroy the current that must be restored on resume.
    this._ablLastActiveDutyPctByDpId = new Map();
  ...
  _buildAliasDefinitions() {
    ...
    if (chargerCats.has(cat)) {
      ...
      //  - ctrl.run / ctrl.chargeEnable -> 100 % wait, restore last charging duty on release
      // The raw mODIFY_STATE datapoint remains available only for service-state transitions.
      try {
        const tplIdLower = String(this.template?.id || '').toLowerCase();
        const mfrLower = String(this.template?.manufacturer || '').toLowerCase();
        ...
        if (isAbl) {
          ...
          const round1 = (v) => {
            if (typeof v !== 'number' || !Number.isFinite(v)) return v;
            // Avoid binary floating-point edge cases such as 52.75 becoming
            // 52.749999..., which would otherwise round down to 52.7 A.
            return Math.round((v + 1e-9) * 10) / 10;
          };
          const floor1 = (v) => {
            if (typeof v !== 'number' || !Number.isFinite(v)) return v;
            // ABL transmits the PWM value in 0.1 % steps. Always quantize down so
            // the generated current limit never exceeds the requested amperage.
            return Math.floor((v + 1e-9) * 10) / 10;
          };

          // IEC 61851 / ABL EVCC2/3 PWM-current conversion.
          // ABL register 0x0014 is the normal EMS control point:
          //  - 100 % means "no current allowed" (pause/wait)
          //  - 10..85 % represent 6..51 A (0.6 A per %)
          //  - >85..96 % represent >52.5..80 A
          //  - 0 % / 5 % and >96..<100 % are special/non-analogue values
          const ABL_WAIT_DUTY_PCT = 100;
          const ABL_MIN_CHARGE_DUTY_PCT = 10;
          const ABL_LOW_RANGE_MAX_DUTY_PCT = 85;
          const ABL_MAX_CHARGE_DUTY_PCT = 96;
          const ablDutyMemoryKey = String(setIcmaxDutyPctDp?.id || 'sET_ICMAX_DUTY_CYCLE_PCT');

          const isAblChargingDutyPct = (value) => {
            const pct = toNum(value);
            return Number.isFinite(pct) &&
              pct >= ABL_MIN_CHARGE_DUTY_PCT &&
              pct <= ABL_MAX_CHARGE_DUTY_PCT;
          };

          const normalizeAblDutyCommandPct = (value) => {
            const pctIn = toNum(value);
            if (!Number.isFinite(pctIn)) return undefined;

            // Generic controllers commonly use 0 % for stop. On ABL, 0 % is not
            // the stop command; 100 % is the documented "no current" value.
            if (pctIn <= 0) return ABL_WAIT_DUTY_PCT;

            // The analogue IEC 61851 current range ends at 96 %. Values 97..99 %
            // are not valid charging-current commands, so fail safe to 100 %.
            if (pctIn > ABL_MAX_CHARGE_DUTY_PCT) return ABL_WAIT_DUTY_PCT;

            // ABL's raw register accepts 8..100 %, but ordinary AC charging starts
            // at 10 % = 6 A. Keep the stable EOS alias inside the analogue range.
            if (pctIn < ABL_MIN_CHARGE_DUTY_PCT) return ABL_MIN_CHARGE_DUTY_PCT;

            return floor1(pctIn);
          };

          const rememberAblActiveDutyPct = (value) => {
            const pct = normalizeAblDutyCommandPct(value);
            if (isAblChargingDutyPct(pct) && this._ablLastActiveDutyPctByDpId) {
              this._ablLastActiveDutyPctByDpId.set(ablDutyMemoryKey, floor1(pct));
            }
            return pct;
          };

          const observeAblAppliedDutyPct = (value) => {
            const pctIn = toNum(value);
            if (!Number.isFinite(pctIn)) return undefined;
            // Keep the charger-side readback separate from the last EMS command.
            // ABL may temporarily apply a lower duty cycle (for example during
            // startup, vehicle reaction or BC3..BC6 derating). Such feedback must
            // not overwrite the value that pause/resume is expected to restore.
            return floor1(pctIn);
          };

          const getLastAblChargingDutyPct = () => {
            const remembered = this._ablLastActiveDutyPctByDpId
              ? this._ablLastActiveDutyPctByDpId.get(ablDutyMemoryKey)
              : undefined;
            if (isAblChargingDutyPct(remembered)) return floor1(Number(remembered));

            const lastRaw = this._lastCommandedValueByDpId && this._lastCommandedValueByDpId.has(ablDutyMemoryKey)
              ? this._lastCommandedValueByDpId.get(ablDutyMemoryKey)
              : undefined;
            if (isAblChargingDutyPct(lastRaw)) return rememberAblActiveDutyPct(lastRaw);

            return ABL_MIN_CHARGE_DUTY_PCT;
          };

          const ablCurrentToDutyPct = (value) => {
            const aIn = toNum(value);
            if (!Number.isFinite(aIn)) return undefined;
            // IEC 61851 analogue AC charging starts at 6 A. A smaller positive
            // current cannot be represented without exceeding the requested limit,
            // therefore use ABL's documented 100 % pause value instead of forcing 6 A.
            if (aIn < 6) return ABL_WAIT_DUTY_PCT;

            const a = Math.min(80, aIn);
            let pct;
            if (a <= 51) {
              // The ABL examples intentionally truncate: 10 A -> 16.6 % (0x00A6),
              // 16 A -> 26.6 % (0x010A). Rounding up would exceed the requested limit.
              pct = floor1(a / 0.6);
            } else {
              const highRangePct = floor1((a / 2.5) + 64);
              // 85.0 % still belongs to the low range and means 51 A. The first
              // valid high-range step is 85.1 % (= 52.75 A). For the unrepresentable
              // gap above 51 A, retain the safe 51 A limit rather than overshooting.
              pct = highRangePct > ABL_LOW_RANGE_MAX_DUTY_PCT
                ? highRangePct
                : ABL_LOW_RANGE_MAX_DUTY_PCT;
            }

            return Math.min(ABL_MAX_CHARGE_DUTY_PCT, Math.max(ABL_MIN_CHARGE_DUTY_PCT, pct));
          };

          const ablDutyPctToCurrentA = (value) => {
            const pctIn = toNum(value);
            if (!Number.isFinite(pctIn)) return undefined;
            const pct = floor1(pctIn);

            // Special values (0 %, 5 %, 100 %) and undefined ranges do not represent
            // a usable analogue AC charging current.
            if (!isAblChargingDutyPct(pct)) return 0;

            const a = (pct > ABL_LOW_RANGE_MAX_DUTY_PCT)
              ? (2.5 * (pct - 64))
              : (0.6 * pct);
          ...
          const toAblBoolean = (value) => {
            if (typeof value === 'boolean') return value;
            if (typeof value === 'number' && Number.isFinite(value)) return value !== 0;
            if (typeof value === 'string') {
              const s = value.trim().toLowerCase();
              if (['true', '1', 'on', 'start', 'enable', 'enabled'].includes(s)) return true;
              if (['false', '0', 'off', 'stop', 'disable', 'disabled'].includes(s)) return false;
            }
            return undefined;
          };
          ...
          // ABL charge control is PWM-duty based.  Keep Ampere as the stable EOS API,
          // but always route the device write to register 0x0014 via
          // sET_ICMAX_DUTY_CYCLE_PCT.  100 % is the normal waiting/pause command.
          if (icmaxDutyPctDp && setIcmaxDutyPctDp) {
            add({
              ...
              fromDevice: (value) => observeAblAppliedDutyPct(value),
              replace: true,
            });
            ...
            add({
              ...
              dpId: icmaxDutyPctDp.id,
              fromDevice: (value) => {
                const pct = observeAblAppliedDutyPct(value);
                return pct === undefined ? undefined : ablDutyPctToCurrentA(pct);
              },
              replace: true,
            });

            // Direct PWM command.  This is the native ABL control axis.
            add({
              relId: this._aliasRelId('ctrl.currentLimitPct'),
              name: 'Charging current limit (ABL PWM duty cycle)',
              role: 'level',
              type: 'number',
              unit: '%',
              rw: 'rw',
              ...
              commandOnlyAlias: true,
              replace: true,
              toDevice: (value) => rememberAblActiveDutyPct(value),
              fromDevice: (value) => observeAblAppliedDutyPct(value),
            });

            // Stable EOS command in amperes.  The hardware still receives PWM percent.
            add({
              relId: this._aliasRelId('ctrl.currentLimitA'),
              name: 'Charging current limit',
              role: 'level.current',
              ...
              commandOnlyAlias: true,
              replace: true,
              toDevice: (value) => rememberAblActiveDutyPct(ablCurrentToDutyPct(value)),
              fromDevice: (value) => {
                const pct = observeAblAppliedDutyPct(value);
                return pct === undefined ? undefined : ablDutyPctToCurrentA(pct);
              },
            ...
            // Normal EMS pause/resume must NOT jump the EVCC state machine to E0/A1.
            // ABL defines 100 % PWM as "no current allowed".  Resume restores the last
            // valid charging duty cycle, or 10 % (= 6 A) when no previous command exists.
            const runAlias = {
              name: 'Run / charging release (ABL PWM)',
              role: 'switch',
              type: 'boolean',
              rw: 'rw',
              kind: 'dp',
              ...
              replace: true,
              toDevice: (value) => {
                const enabled = toAblBoolean(value);
                if (enabled === undefined) return undefined;
                if (enabled === false) {
                  // Capture a direct/raw PWM command before replacing it with 100 %.
                  getLastAblChargingDutyPct();
                  return ABL_WAIT_DUTY_PCT;
                }
                return getLastAblChargingDutyPct();
              },
              fromDevice: (value) => isAblChargingDutyPct(value),
            };
            ...
            add({
              relId: this._aliasRelId('ctrl.chargeEnable'),
              ...runAlias,
              name: 'Charging release (ABL PWM)',
            });

            add({
              relId: this._aliasRelId('r.chargingReleased'),
              name: 'Charging released by ABL PWM setpoint',
              role: 'indicator',
              type: 'boolean',
              rw: 'ro',
              kind: 'dp',
            ...
            add({
              ...
              name: 'Waiting because ABL PWM is 100 percent',
              role: 'indicator',
              type: 'boolean',
              rw: 'ro',
              kind: 'dp',
              ...
              fromDevice: (value) => {
                const pct = toNum(value);
                return Number.isFinite(pct) ? pct >= ABL_WAIT_DUTY_PCT : undefined;
              },
              replace: true,
            });
          }

          // mODIFY_STATE remains available as an expert/raw datapoint for service
          // transitions (A1/E0/E2), but it is deliberately no longer used by the
          // normal EOS run/stop control.
        }
      } catch (e) {
        // Never break alias generation due to special-case logic
      }
  ...
  async registerDynamicAlias(def) {
  ...
  _rememberCommandedValue(dpId, deviceValue) {
    ...
    try {
      ...
      // ABL eMH1: remember only valid analogue PWM charge commands (10..96 %).
      // The normal pause command is 100 % and must never overwrite the last active
      // current that ctrl.run/ctrl.chargeEnable restores on resume.
      try {
        const tplIdLower = String(this.template?.id || '').toLowerCase();
        const mfrLower = String(this.template?.manufacturer || '').toLowerCase();
        const isAblEmh1 = (mfrLower === 'abl' || tplIdLower.startsWith('evcs.abl.')) &&
          key === 'sET_ICMAX_DUTY_CYCLE_PCT';
        const dutyPct = Number(valueToRemember);
        if (isAblEmh1 && Number.isFinite(dutyPct) && dutyPct >= 10 && dutyPct <= 96) {
          const normalized = Math.floor((dutyPct + 1e-9) * 10) / 10;
          if (this._ablLastActiveDutyPctByDpId) this._ablLastActiveDutyPctByDpId.set(key, normalized);
        }
      } catch (_) {
        // optional ABL command-memory update
      }
  ...
  _enqueueWrite(ackRelId, dp, deviceValue, ackVal, meta = {}) {
    ...
    if (!entry) {
      ...
      if (incomingAutomatic && (existingUserCommand || !existingAutomatic)) {
        if (ackRelId) entry.ackByRelId.set(String(ackRelId), ackVal);
        return;
      }

      entry.dp = dp;
      entry.deviceValue = deviceValue;
      entry.meta = Object.assign({}, existingMeta, meta || {}, {
