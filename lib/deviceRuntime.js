...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    if (batteryCats.has(cat)) {
      ...
      if (setActivePowerDp) {
        const dedicatedChargePowerDp = getAnyById('sET_CHARGE_POWER');
        const dedicatedDischargePowerDp = getAnyById('sET_DISCHARGE_POWER');
        const useDedicatedSplitPowerDps = !!(dedicatedChargePowerDp && dedicatedDischargePowerDp
          && (dedicatedChargePowerDp.rw === 'rw' || dedicatedChargePowerDp.rw === 'wo')
          && (dedicatedDischargePowerDp.rw === 'rw' || dedicatedDischargePowerDp.rw === 'wo'));
        ...
        // Default convention: charge power is positive on the public alias and maps
        // to a negative signed setpoint; discharge maps to a positive signed setpoint.
        // If a template exposes dedicated virtual charge/discharge writepoints (Sungrow
        // External EMS), use them for the write path so the direction cannot be swapped
        // by a sign convention mismatch. The signed setpoint remains the readback mirror
        // so dashboards still see a coherent ctrl.powerSetpointW value.
        add({
          ...
          kind: 'dp',
          dpId: setActivePowerDp.id,
          writeDpId: useDedicatedSplitPowerDps ? dedicatedChargePowerDp.id : setActivePowerDp.id,
          mirrorSignedDpId: useDedicatedSplitPowerDps ? setActivePowerDp.id : undefined,
          toMirrorDevice: (v) => {
            const n = Number(v);
          ...
          toDevice: (v) => {
            const n = Number(v);
            if (!Number.isFinite(n)) return v;
            return useDedicatedSplitPowerDps ? Math.abs(n) : -Math.abs(n);
          },
          fromDevice: (v) => {
            const n = Number(v);
            if (!Number.isFinite(n)) return undefined;
        ...
        add({
          ...
          kind: 'dp',
          dpId: setActivePowerDp.id,
          writeDpId: useDedicatedSplitPowerDps ? dedicatedDischargePowerDp.id : setActivePowerDp.id,
          mirrorSignedDpId: useDedicatedSplitPowerDps ? setActivePowerDp.id : undefined,
          toMirrorDevice: (v) => {
            const n = Number(v);
            if (!Number.isFinite(n)) return v;
  ...
  async _flushWriteQueueOnce() {
    ...
    try {
      ...
      if (entry.meta && entry.meta.mirrorSignedDpId) {
        const mirrorDp = this._getDpById(entry.meta.mirrorSignedDpId);
        if (mirrorDp) await this._ackWrittenValue(mirrorDp, entry.meta.mirrorSignedValue);
      }
      ...
      try {
        ...
        if (this._isWriteQueueEnabled()) {
          this._enqueueWrite(relId, aliasTargetDp, toDev, state.val, {
            bypassUnsupportedCooldown: true,
            isUserCommand: true,
            mirrorSignedDpId: mirrorSignedDp && mirrorSignedDp.id ? mirrorSignedDp.id : undefined,
            mirrorSignedValue: mirrorSignedDp ? mirrorSignedVal : undefined,
          });
          return;
        }
