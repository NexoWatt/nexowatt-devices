    ...
    if (chargerCats.has(cat)) {
      ...
      try {
        ...
        if (isAbl) {
          ...
          // IEC 61851 PWM/current conversion used by the ABL EVCC2/3 API.
          // 100 % has the ABL-specific meaning "no current allowed".
          const ablCurrentToDutyPct = (value) => {
            ...
            const a = Math.min(80, Math.max(6, aIn));
            const pct = (a <= 51)
              ? (a / 0.6)
              : ((a / 2.5) + 64);

            // 96 % is the upper analogue-current boundary. 100 % is reserved for stop.
            return round1(Math.min(96, Math.max(10, pct)));
          };
          ...
          const ablDutyPctToCurrentA = (value) => {
            ...
            // IEC 61851 defines 85 % as the upper boundary of the 0.6 A/% range.
            // The high-current formula starts strictly above 85 %.
            const a = (pctIn > 85)
              ? (2.5 * (pctIn - 64))
              : (0.6 * pctIn);
