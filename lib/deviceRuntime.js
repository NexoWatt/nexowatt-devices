    ...
    if (chargerCats.has(cat)) {
      ...
      try {
        ...
        if (isAlfenAce) {
          ...
          const alfenHints = (this.template && this.template.driverHints && this.template.driverHints.alfen) || {};
          const runCurrentRaw = Number(alfenHints.runCurrentA ?? alfenHints.defaultEnableCurrentA ?? 16);
          ...
          if (energyDeliveredSumDp || energyConsumedSumDp) {
            add({
              ...
              role: 'value.energy',
              type: 'number',
              unit: (energyDeliveredSumDp && energyDeliveredSumDp.unit) || (energyConsumedSumDp && energyConsumedSumDp.unit) || 'Wh',
              rw: 'ro',
              kind: 'computed',
              replace: true,
              get: (values) => {
                const delivered = energyDeliveredSumDp ? toNum(values && values[energyDeliveredSumDp.id]) : undefined;
                if (Number.isFinite(delivered)) return delivered;
                const consumed = energyConsumedSumDp ? toNum(values && values[energyConsumedSumDp.id]) : undefined;
    ...
    if (aliasDef) {
      ...
      try {
        ...
        if (this._isWriteQueueEnabled()) {
          this._enqueueWrite(relId, aliasTargetDp, toDev, state.val);
          return;
        }
    ...
    const tick = async () => {
      ...
      try {
        ...
        for (const t of activeTargets) {
          ...
          const activeIfCommandDpId = String(t.activeIfCommandDpId || '').trim();
          if (activeIfCommandDpId) {
            let gateVal = NaN;
            if (this._lastCommandedValueByDpId && this._lastCommandedValueByDpId.has(activeIfCommandDpId)) {
              gateVal = Number(this._lastCommandedValueByDpId.get(activeIfCommandDpId));
            }
            const gateGt = Number(t.activeIfCommandValueGt);
            if (!Number.isFinite(gateVal)) continue;
            if (Number.isFinite(gateGt) && !(gateVal > gateGt)) continue;
          }

          if (cfg.requireEverWritten) {
          ...
          try {
            ...
            this.adapter.log.debug(`[${this.cfg.id}] SetpointKeepalive: refreshed ${setDp.id}=${setVal}${reason ? ` (${reason})` : ''}`);
          } catch (e) {
            if (this._isSuppressedUnsupportedWriteError(e)) {
