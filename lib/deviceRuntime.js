...
class DeviceRuntime {
        ...
        if (isAlfenAce) {
          ...
          if (primaryCurrentReadDp) {
            add({
              ...
              name: preferScnControl ? 'SCN actual applied max current' : 'Actual applied max current',
              role: 'value.current',
              type: 'number',
              unit: primaryCurrentReadDp.unit || 'A',
              rw: 'ro',
              kind: 'dp',
              dpId: primaryCurrentReadDp.id,
              replace: true,
            });
          }
          ...
          if (primaryValidTimeDp) {
            add({
              ...
              name: preferScnControl ? 'SCN current-limit valid time remaining' : 'Current-limit valid time remaining',
              role: 'value',
              type: 'number',
              unit: primaryValidTimeDp.unit || 's',
              rw: 'ro',
              kind: 'dp',
              dpId: primaryValidTimeDp.id,
              replace: true,
              fromDevice: (v) => {
                let n = toNum(v);
                if (!Number.isFinite(n)) return undefined;
          ...
          if (preferScnControl && accountedDp) {
            add({
          ...
          if (preferScnControl && scnMaxCurrentEnableDp) {
            add({
  ...
  _alfenMirrorCommandDpIds(dpId) {
    ...
    // Alfen ACE can expose one physical station through multiple Modbus server
    // addresses. The user-facing Socket-1 aliases are our stable API, but the
    // charger may be configured to account for Socket-2 or SCN maximum-current
    // registers instead. Mirror command memory into those writable datapoints so
    // the 5 s watchdog refreshes the same command on all supported IDs, not only
    // the primary socket write that triggered the command.
    if (id === 'sET_CHARGING_CURRENT') {
      return ['s2_sET_CHARGING_CURRENT', 'sCN_MAX_CURRENT'];
    }
    if (id === 'cHARGE_USING_PHASES') {
      return ['s2_cHARGE_USING_PHASES'];
    }
    return [];
  }
