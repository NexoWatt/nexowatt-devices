...
class DeviceRuntime {
  ...
  _buildAliasDefinitions() {
    ...
    if (batteryCats.has(cat)) {
      ...
      const computeBatteryPowerW = (values) => {
        ...
        // Prefer separate charge/discharge power datapoints if available (both should be positive).
        const chargeDp = (battChargePowerDp && battChargePowerDp.role === 'value.power') ? battChargePowerDp : undefined;
        const dischargeDp = (battDischargePowerDp && battDischargePowerDp.role === 'value.power') ? battDischargePowerDp : undefined;
        const ch = chargeDp ? asNumber(values[chargeDp.id]) : undefined;
        const dis = dischargeDp ? asNumber(values[dischargeDp.id]) : undefined;
        if (ch !== undefined || dis !== undefined) {
          const chv = ch ?? 0;
          const disv = dis ?? 0;
          // Net power: discharge minus charge
          return disv - chv;
        }
  ...
  async _persistSetpointValueIfNeeded(dp, deviceValue) {
    ...
    await this.adapter.setStateAsync(this._setpointMemoryRelId(dp.id), { val: out, ack: true }).catch(() => {});
  }
