...
class DeviceRuntime {
  constructor(adapter, deviceConfig, template, globalConfig) {
    ...
    // unchanged setState() calls on every poll cycle.
    this._stateCache = new Map(); // relId -> { val, ack }
  ...
  async _setStateCached(relId, val, ack = true) {
    ...
    if (prev && prev.ack === nextAck && this._sameStateValue(prev.val, nextVal)) {
      return false;
    }

    try {
      await this.adapter.setStateAsync(key, { val: nextVal, ack: nextAck });
      this._stateCache.set(key, { val: nextVal, ack: nextAck });
      return true;
    } catch (_) {
      return false;
    }
  ...
  async _setHeartbeatOnline(nextOnline) {
    ...
    this._hbOnline = b;
    await this._setStateCached(this._hbRelId('online'), b, true);
  }
  ...
  async _tickHeartbeatFromIncomingData(sourceStamp) {
    ...
    await this._setStateCached(this._hbRelId('heartbeat'), this._hbCounter, true);
    await this._setStateCached(this._hbRelId('lastSeenMs'), this._hbLastSeen || now, true);
  }
    ...
    if (chargerCats.has(cat)) {
      ...
      try {
        ...
        if (isAlfenAce) {
          ...
          const alfenHints = Object.assign(
            {},
            (this.template && this.template.alfen && typeof this.template.alfen === 'object') ? this.template.alfen : {},
            (this.template && this.template.driverHints && this.template.driverHints.alfen && typeof this.template.driverHints.alfen === 'object') ? this.template.driverHints.alfen : {},
            (this.template && this.template.driverHints && this.template.driverHints.modbus && this.template.driverHints.modbus.alfen && typeof this.template.driverHints.modbus.alfen === 'object') ? this.template.driverHints.modbus.alfen : {}
          );
          // For Alfen ACE, a plain run/enable command must not force 16 A over a PV/minimum
          // setpoint.  Use 6 A as the safe default release current; Boost/EMS can still write
          // a higher current explicitly through ctrl.currentLimitA or ctrl.powerLimitW.
          ...
          const preferScnControl = !!(scnAllCurrentDp && (alfenControlMode === 'scn' || tplIdLower.includes('.scn.')));
          const primaryCurrentWriteDp = preferScnControl ? scnAllCurrentDp : setCurrentDp;
          // Readback and command are separate on Alfen: 1206..1207 is the actual applied
          // current, while 1210..1211 is the writable Max Current setpoint.  Do not treat
          // the write setpoint as the live readback.
          const primaryCurrentReadDp = preferScnControl ? (scnActualCurrentL1Dp || scnAllCurrentDp) : (appliedCurrentDp || setCurrentDp);
          ...
          if (appliedCurrentDp) {
            add({
              ...
              role: 'value.current',
              type: 'number',
              unit: appliedCurrentDp.unit || 'A',
              rw: 'ro',
              kind: 'dp',
              dpId: appliedCurrentDp.id,
            });
          }
          ...
          if (validTimeDp) {
            add({
              ...
              name: 'Current-limit valid time remaining',
              role: 'value',
              type: 'number',
              unit: validTimeDp.unit || 's',
              rw: 'ro',
              kind: 'dp',
              dpId: validTimeDp.id,
              fromDevice: (v) => {
                let n = toNum(v);
                if (!Number.isFinite(n)) return undefined;
                // Some ACE control-block readbacks expose UINT32 valid-time with the
                // word order opposite to FLOAT32 values.  Symptom seen in the field:
                // 76 s arrives as 4,980,736 (=76<<16).  Normalize this for the alias.
                if (n > 86400 && Math.trunc(n) % 65536 === 0) {
                  const swapped = n / 65536;
                  if (Number.isFinite(swapped) && swapped >= 0 && swapped <= 86400) n = swapped;
                }
          ...
          if (phasesDp) {
            add({
              ...
              toDevice: (v) => {
                ...
                // Do NOT coerce false/0/undefined/NaN to 3 phases; that could send an
                // unintended EMS write when a UI toggle or automation writes 0/false.
          ...
          if (primaryCurrentWriteDp && (primaryCurrentWriteDp.rw === 'rw' || primaryCurrentWriteDp.rw === 'wo')) {
            ...
            const runAlias = {
              ...
              commandOnlyAlias: true,
              toDevice: (v) => {
                if (typeof v === 'boolean') return v ? runCurrentA : stopCurrentA;
                const n = Number(v);
                if (Number.isFinite(n)) return n ? runCurrentA : stopCurrentA;
                return v;
              },
              fromDevice: (v) => {
                const n = Number(v);
            ...
            // Alfen exposes EMS control as current (A), while the EOS EVCS controller may
            // also send a power setpoint (W), e.g. for Min+PV.  Convert W -> A so a 6 A
            // minimum setpoint is not overwritten by the boolean run alias.
            const nominalVoltageRaw = Number(alfenHints.nominalVoltageV ?? alfenHints.nominalPhaseVoltageV ?? 230);
            const nominalVoltageV = Number.isFinite(nominalVoltageRaw) && nominalVoltageRaw > 0 ? nominalVoltageRaw : 230;
            const getLastCommandedPhaseCount = () => {
              try {
                if (phasesDp && this._lastCommandedValueByDpId && this._lastCommandedValueByDpId.has(String(phasesDp.id))) {
                  const p = Math.trunc(Number(this._lastCommandedValueByDpId.get(String(phasesDp.id))));
                  if (p === 1 || p === 3) return p;
                }
              } catch (_) {}
              return Number(alfenHints.defaultPhaseMode || alfenHints.defaultPhases || 3) === 1 ? 1 : 3;
            };
            add({
              relId: this._aliasRelId('ctrl.powerLimitW'),
              ...
              toDevice: (v) => {
                const w = Number(v);
                if (!Number.isFinite(w)) return v;
                if (w <= 0) return 0;
                const phases = getLastCommandedPhaseCount();
                let amps = Math.round(w / Math.max(1, nominalVoltageV * phases));
                // IEC 61851 minimum is 6 A.  If a positive power target calculates lower,
                // release with 6 A instead of falling back to the generic 16 A run command.
              ...
              fromDevice: (v) => {
                ...
                return Math.round(a * nominalVoltageV * getLastCommandedPhaseCount());
              },
            });

            add({
              relId: this._aliasRelId('r.powerLimitW'),
              name: preferScnControl ? 'SCN commanded power limit' : 'Commanded power limit',
              role: 'value.power',
              type: 'number',
              unit: 'W',
              rw: 'ro',
            ...
            add({
              ...
              get: (values) => {
                const cmd = this._lastCommandedValueByDpId && this._lastCommandedValueByDpId.has(String(primaryCurrentWriteDp.id))
                  ...
                  : undefined;
                if (Number.isFinite(cmd)) return cmd > 0.1;

                const validS = validTimeDp ? toNum(values && values[validTimeDp.id]) : undefined;
                ...
                const readback = primaryCurrentReadDp ? toNum(values && values[primaryCurrentReadDp.id]) : undefined;
                return Number.isFinite(readback) ? readback > 0.1 : undefined;
              },
            });
          }
        }
        ...
        if (isAbl) {
          ...
          if (currentDpsAbl.length) {
            add({
              ...
              kind: 'computed',
              get: (values) => {
                const totalA = getTotalCurrentA(values);
                if (!Number.isFinite(totalA)) return undefined;
                return totalA;
              }
            });
          }
          ...
          if (!chargingPowerDp && currentDpsAbl.length) {
            add({
              ...
              get: (values) => {
                const totalA = getTotalCurrentA(values);
                if (!Number.isFinite(totalA)) return undefined;
                return Math.round(totalA * nominalPhaseVoltageV);
              }
            });
  ...
  async _updateAliases(values, ctx) {
    ...
    const isConnectionAlias = (relId) => (
      String(relId || '').endsWith('.aliases.comm.connected') ||
      String(relId || '').endsWith('.aliases.comm.lastError') ||
      String(relId || '').endsWith('.aliases.alarm.offline')
    );
  ...
  async _setError(e) {
    ...
    if (lower.includes('illegal data address') || lower.includes('exception code') || lower.includes('illegal function')) {
      ...
      if (isAlfen) {
        addHint(
          `Alfen-specific: the template already uses the ACE protocol addresses (document register minus 1) and fixed Unit-IDs. ` +
          `If UID1 FC16@1209 or UID1 FC3 1199-1214 is rejected, the charger is answering but the Socket control/status block is not exposed in the current ACE configuration/firmware profile.`
        );
        if (lower.includes('set_charging_current') || lower.includes('fc16@1209') || lower.includes('1199-1214')) {
          addHint(
            `Enable Active Load Balancing/EMS, set TCP/IP EMS Control Mode to Socket for Unit-ID 1, enable sockets, and allow writing maximum currents. ` +
  ...
  _isAlfenRunAliasRelId(relId) {
    const id = String(relId || '');
    return id.endsWith('.aliases.ctrl.run') || id.endsWith('.aliases.ctrl.chargeEnable');
  }

  _isAlfenCurrentCommandAliasRelId(relId) {
    const id = String(relId || '');
    return id.endsWith('.aliases.ctrl.currentLimitA') || id.endsWith('.aliases.ctrl.powerLimitW');
  }

  _coerceBooleanCommandValue(v) {
    if (typeof v === 'boolean') return v;
  ...
  async _resolveNumericRelState(relId) {
    if (!relId) return undefined;
    const st = await this.adapter.getStateAsync(relId).catch(() => null);
    if (!st || st.val === undefined || st.val === null) return undefined;
    const n = Number(typeof st.val === 'string' ? st.val.replace(',', '.') : st.val);
    return Number.isFinite(n) ? n : undefined;
  }

  async _resolveAlfenReleaseCurrentA() {
    const dpId = 'sET_CHARGING_CURRENT';
    const clamp = (v) => {
      const n = Number(v);
      if (!Number.isFinite(n)) return undefined;
      if (n <= 0.1) return undefined;
      return Math.max(0, Math.min(80, Math.round(n)));
    };

    // Prefer the visible EMS command.  This lets Min+PV write 6 A and then toggle run=true
    // without the run alias replacing it with the default enable current.
    let v = clamp(await this._resolveNumericRelState(this._aliasRelId('ctrl.currentLimitA')));
    if (v !== undefined) return v;

    // Power command alias maps to the same current register; if present, convert it back
    // roughly with 230 V and the last commanded phase count.
    const pW = await this._resolveNumericRelState(this._aliasRelId('ctrl.powerLimitW'));
    if (Number.isFinite(pW) && pW > 0) {
      let phases = 3;
      try {
        if (this._lastCommandedValueByDpId && this._lastCommandedValueByDpId.has('cHARGE_USING_PHASES')) {
          const p = Math.trunc(Number(this._lastCommandedValueByDpId.get('cHARGE_USING_PHASES')));
          if (p === 1 || p === 3) phases = p;
        }
      } catch (_) {}
      v = clamp(Math.round(pW / (230 * phases)));
      if (v !== undefined && v < 6) v = 6;
      if (v !== undefined) return v;
    }

    if (this._lastCommandedValueByDpId && this._lastCommandedValueByDpId.has(dpId)) {
      v = clamp(this._lastCommandedValueByDpId.get(dpId));
      if (v !== undefined) return v;
    }

    v = clamp(await this._resolveNumericRelState(this._setpointMemoryRelId(dpId)));
    if (v !== undefined) return v;

    const dp = this._getDpById(dpId);
    if (dp) {
      v = clamp(await this._resolveNumericRelState(this.relStateId(dp)));
      if (v !== undefined) return v;
    }

    const modHints = this.template?.driverHints?.modbus || {};
    const alfenHints = (modHints.alfen && typeof modHints.alfen === 'object') ? modHints.alfen : {};
    const raw = Number(alfenHints.runCurrentA ?? alfenHints.defaultEnableCurrentA ?? this.template?.alfen?.runCurrentA ?? this.template?.alfen?.defaultEnableCurrentA ?? 6);
    return Number.isFinite(raw) && raw > 0 ? Math.max(1, Math.min(80, Math.round(raw))) : 6;
  }
  ...
  _enqueueWrite(ackRelId, dp, deviceValue, ackVal, meta = {}) {
    ...
    if (this._isWriteTemporarilyUnsupported(dp.id)) {
    ...
    if (!entry) {
      ...
      // Alfen ACE: run/chargeEnable=true writes the same Max Current register as the
      // actual current/power setpoint aliases.  When a controller sends Min+PV=6 A and
      // then toggles run=true in the same burst, do not let the boolean release alias
      // overwrite the explicit current/power command with the default release current.
      const isAlfenSetCurrent = this._isAlfenTemplate() && String(key) === 'sET_CHARGING_CURRENT';
      const newIsRunAlias = isAlfenSetCurrent && meta && this._isAlfenRunAliasRelId(meta.aliasRelId);
      const oldIsExplicitLimit = isAlfenSetCurrent && entry.meta && this._isAlfenCurrentCommandAliasRelId(entry.meta.aliasRelId);
      const oldVal = Number(entry.deviceValue);
      const newVal = Number(deviceValue);
      if (newIsRunAlias && oldIsExplicitLimit && Number.isFinite(oldVal) && oldVal > 0.1 && Number.isFinite(newVal) && newVal > 0.1) {
        // Keep the explicit Min+PV/current/power limit already queued.
        entry.meta = Object.assign({}, entry.meta || {}, { ignoredRunAliasRelId: meta.aliasRelId });
      } else {
        entry.dp = dp;
  ...
  async _flushWriteQueueOnce() {
    ...
    try {
      ...
      await this._persistSetpointValueIfNeeded(entry.dp, entry.deviceValue).catch(() => {});
  ...
  async handleStateChange(fullId, state) {
    ...
    if (aliasDef) {
      ...
      try {
        ...
        if (this._isWriteTemporarilyUnsupported(aliasTargetDp.id)) {
          this._logSkippedUnsupportedWrite(aliasTargetDp.id);
          return;
        }

        let toDev = (typeof aliasDef.toDevice === 'function') ? aliasDef.toDevice(state.val) : state.val;
        // Alfen ACE: ctrl.run/ctrl.chargeEnable=true is a release command and must not
        // overwrite a Min+PV/current/power command with the static default.  Resolve the
        // active current from ctrl.currentLimitA / last command / setpoint memory first.
        if (this._isAlfenTemplate() && aliasTargetDp && String(aliasTargetDp.id) === 'sET_CHARGING_CURRENT' && this._isAlfenRunAliasRelId(relId)) {
          const b = this._coerceBooleanCommandValue(state.val);
          if (b === true) {
            const dyn = await this._resolveAlfenReleaseCurrentA();
            if (Number.isFinite(dyn) && dyn > 0.1) toDev = dyn;
          } else if (b === false) {
            toDev = 0;
          }
        }
        ...
        if (this._isWriteQueueEnabled()) {
          this._enqueueWrite(relId, aliasTargetDp, toDev, state.val, { aliasRelId: relId });
          return;
        }
    ...
    try {
      ...
      // Persist last setpoint for restore-on-start/reconnect (best-effort)
      await this._persistSetpointValueIfNeeded(dp, state.val).catch(() => {});
  ...
  async _seedSetpointKeepaliveFromCommandAliases() {
    ...
    const modHints = this.template?.driverHints?.modbus || {};
    const alfenHints = (modHints && modHints.alfen && typeof modHints.alfen === 'object') ? modHints.alfen : {};
    ...
    if (this._isAlfenTemplate()) {
      // A positive run/chargeEnable alias is only a release signal.  It must not overwrite
      // an EMS/min+PV current command with a fixed 16 A default.  Default to the EV minimum
      // of 6 A unless the template explicitly says otherwise.
      const runCurrentRaw = Number(alfenHints.runCurrentA ?? alfenHints.defaultEnableCurrentA ?? 6);
      ...
      if (targets.has('sET_CHARGING_CURRENT')) {
        ...
        const aliasPowerW = toNum(await readAlias('ctrl.powerLimitW'));
        ...
        } else if (Number.isFinite(aliasPowerW) && aliasPowerW > 0.1) {
          // Best-effort restore for UIs/controllers that drive EVCS by W instead of A.
          // Use 3 phases by default; if the phase alias is seeded below, the next command
          // refresh will use that phase command.
          currentSeed = Math.round(aliasPowerW / (230 * 3));
          if (currentSeed > 0 && currentSeed < 6) currentSeed = 6;
          currentSource = 'aliases.ctrl.powerLimitW';
        } else if (runAlias === true || enableAlias === true) {
          // run/chargeEnable is a release flag, not a 16 A command.  Use the safe minimum
          // unless an explicit current/power alias already provided a value.
        ...
        if (Number.isFinite(currentSeed)) {
          ...
          if (currentSeed > 80) currentSeed = 80;
      ...
      if (targets.has('cHARGE_USING_PHASES')) {
        let phaseSeed = toNum(await readState(this._setpointMemoryRelId('cHARGE_USING_PHASES')));
        let phaseSource = 'info.setpoints.cHARGE_USING_PHASES';
        if (!(phaseSeed === 1 || phaseSeed === 3)) {
          phaseSeed = toNum(await readAlias('ctrl.phaseMode'));
          phaseSource = 'aliases.ctrl.phaseMode';
        }
        if (!(phaseSeed === 1 || phaseSeed === 3)) {
          phaseSeed = toNum(await readDp('cHARGE_USING_PHASES'));
          phaseSource = 'cHARGE_USING_PHASES state';
        }
        if (phaseSeed === 1 || phaseSeed === 3) {
