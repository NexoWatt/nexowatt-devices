...
class DeviceRuntime {
  ...
  async _setStateCached(relId, val, ack = true) {
    ...
    if (prev && prev.ack === nextAck && this._sameStateValue(prev.val, val)) {
      return false;
    }

    try {
      await this.adapter.setStateAsync(key, { val, ack: nextAck });
      this._stateCache.set(key, { val, ack: nextAck });
      return true;
    } catch (_) {
      return false;
    }
    ...
    if (chargerCats.has(cat)) {
      ...
      try {
        ...
        if (isAlfenAce) {
          ...
          const primaryCurrentReadDp = primaryCurrentWriteDp;
        ...
        if (isKebaP40) {
          ...
          if (kebaSetEnableDp && (kebaSetEnableDp.rw === 'rw' || kebaSetEnableDp.rw === 'wo')) {
            add({
              ...
              rw: 'rw',
              kind: 'dp',
              dpId: kebaStatusDp ? kebaStatusDp.id : kebaSetEnableDp.id,
          ...
          if (kebaSetUnlockDp && (kebaSetUnlockDp.rw === 'rw' || kebaSetUnlockDp.rw === 'wo')) {
            add({
              ...
              rw: 'rw',
              kind: 'dp',
              dpId: kebaCableDp ? kebaCableDp.id : kebaSetUnlockDp.id,
          ...
          if (kebaActivateFastChargingDp && (kebaActivateFastChargingDp.rw === 'rw' || kebaActivateFastChargingDp.rw === 'wo')) {
            add({
              ...
              rw: 'rw',
              kind: 'dp',
              dpId: kebaFastChargingStateDp ? kebaFastChargingStateDp.id : kebaActivateFastChargingDp.id,
  ...
  async _setError(e) {
    ...
    const keepConnectionState = isWriteOperation && !isTransport;

    // A rejected write value/register is not the same as a lost TCP/Modbus connection.
    // Keep comm.connected stable for non-transport write errors so one bad setpoint
    // does not make the UI look offline. Poll/read errors still mark the device offline.
    ...
    const tick = async () => {
      ...
      try {
        ...
        for (const t of activeTargets) {
          ...
          try {
            if (this._isWriteQueueEnabled()) {
              this._enqueueWrite(null, setDp, setVal, setVal);
            } else {
              await this.driver.writeDatapoint(setDp, setVal);
              this._recordWrite(setDp.id);
