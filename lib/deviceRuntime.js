          ...
          if (primaryCurrentWriteDp && (primaryCurrentWriteDp.rw === 'rw' || primaryCurrentWriteDp.rw === 'wo')) {
            ...
            add({
              ...
              get: (values) => {
                ...
                // command requested by EOS. Alfen exposes this explicitly: socket
                // register 1214 says whether the setpoint is accounted; SCN mode uses
                // its own remaining-valid-time / enable registers. If these readbacks
                // say "not accounted", do not report released just because a command
                // value exists in ioBroker.
                const validS = primaryValidTimeDp ? toNum(values && values[primaryValidTimeDp.id]) : (validTimeDp ? toNum(values && values[validTimeDp.id]) : undefined);
