...
class DeviceRuntime {
  ...
  async _setError(e) {
    ...
    if (lower.includes('illegal data address') || lower.includes('exception code') || lower.includes('illegal function')) {
      const tplIdLower = String(this.template?.id || '').toLowerCase();
      const mfrLower = String(this.template?.manufacturer || '').toLowerCase();
      const isAlfen = mfrLower === 'alfen' || tplIdLower.includes('alfen');
          ...
          if (entry.attempts >= maxAttempts) {
            ...
            if (looksPermanent) {
              ...
              if (cooldownMs > 0 && entry.dp && entry.dp.id) {
                try { this._setpointKeepaliveUnsupportedUntilByDpId.set(String(entry.dp.id), Date.now() + cooldownMs); } catch (_) {}
              }
            }
          }
        }
  ...
  async handleStateChange(fullId, state) {
    ...
    if (aliasDef) {
      ...
      try {
        ...
        // Persist last setpoint for restore-on-start/reconnect (best-effort)
        await this._persistSetpointValueIfNeeded(dp, toDev).catch(() => {});
  ...
  async _restoreSetpointsOnce(reason = '') {
    ...
    try {
      ...
      for (const rawId of cfg.dpIds) {
        ...
        if (mem && mem.val !== null && mem.val !== undefined) val = mem.val;
      ...
      if (actions.length) {
        this.adapter.log.info(`[${this.cfg.id}] Restored setpoints${reason ? ` (${reason})` : ''}: ${actions.join(', ')}`);
      } else {
  ...
  async _startSetpointKeepalive() {
    ...
    const tick = async () => {
      ...
      try {
        ...
        for (const t of cfg.targets) {
          ...
          // Use the currently stored setpoint value (readback) as refresh value.
          const setState = await this.adapter.getStateAsync(this.relStateId(setDp)).catch(() => null);
          const setVal = setState && setState.val !== undefined ? Number(setState.val) : NaN;
    ...
    this.adapter.log.info(`[${this.cfg.id}] SetpointKeepalive enabled: targets=[${cfg.targets.map(t => t.setpointDpId).join(', ')}], periodMs=${periodMs}`);
  }
